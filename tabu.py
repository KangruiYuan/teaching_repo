...
# 读取坐标文件
filename = Path(__file__).parent / "data" / "test.txt"
city_num = []  # 城市编号
city_location = []  # 城市坐标
...
# 计算邻接矩阵
dis = [[0.0] * city_count for i in range(city_count)]  # 初始化
for i in range(city_count):
    for j in range(city_count):
        if i != j:
            dis[i][j] = math.sqrt(
                (city_location[i][0] - city_location[j][0]) ** 2
